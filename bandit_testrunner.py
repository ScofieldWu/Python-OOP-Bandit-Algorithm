...
class BanditTestRunner():
    ...
    def perform_runs(self, timesteps, runs, stationarity = True):
        """
        ...
        for run in tqdm( range(0, runs) ):
            ...
            for timestep in range(0,timesteps):
                for agent_id, agent in enumerate(self.agents):
                    ...
                    # Give non-stationarity to the environment
                    # the default is stationary
                    if not stationarity:
                        self.bandit_env.random_walk_action_values(mu = 0, sigma = 0.01)
    ...
    def visualize_results(self, save_filename, title, rewards_histories,
        ...
        labels = []
        for agent in self.agents:
            labels.append(agent.policy.__str__() + " -- " + agent.__str__())
